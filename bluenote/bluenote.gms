...
PARAMETER	seds_units(source,sector,r,yr,*)	"Complete EIA SEDS data, with units as domain",
		...
		seds_oil_demand				"Total oil demand by the transportation sector in each region-yr from SEDS";
...
SET fds_e(demsec) "Non energy final demand categories" / ind, com, res, air, train, water, trk, ldv/;

SET mapdems(*,demsec) "Mapping between SEDS and IO demanding sectors" /
$INCLUDE maps%sep%mapseds_full_disagg3.map
...
execute_unload '%system.fp%gdx%sep%id0test_before.gdx', id0=id0;

PARAMETER bea_dmd(yr,r,*)	"demand for oil in each region: intermediate demamnd + consumption + investment + govt";
*bea_dmd(yr,r,"oil") = sum(ds,id0(yr,r,"oil",ds)) + cd0(yr,r,"oil") + i0(yr,r,"oil");
bea_dmd(yr,r,"oil") = sum(ds$(not sameas(ds,"air") AND not sameas(ds,"trn") AND not sameas(ds,"wtt") AND not sameas(ds,"trk") AND not sameas(ds,"grd") AND not sameas(ds,"pip") AND not sameas(ds,"otr")),id0(yr,r,"oil",ds)) + cd0(yr,r,"oil") + i0(yr,r,"oil");


* id0(yr,r,"oil","air") = id0(yr,r,"oil","air") + seds_oil_demand(yr,r,"oil","air") * (id0(yr,r,"oil","air") / bea_dmd(yr,r,"oil"));
* id0(yr,r,"oil","trn") = id0(yr,r,"oil","trn") + seds_oil_demand(yr,r,"oil","train") * (id0(yr,r,"oil","trn") / bea_dmd(yr,r,"oil"));
* id0(yr,r,"oil","trk") = id0(yr,r,"oil","trk") + seds_oil_demand(yr,r,"oil","trk") * (id0(yr,r,"oil","trk") / bea_dmd(yr,r,"oil"));
* id0(yr,r,"oil","wtt") = id0(yr,r,"oil","wtt") + seds_oil_demand(yr,r,"oil","water") * (id0(yr,r,"oil","wtt") / bea_dmd(yr,r,"oil"));
* id0(yr,r,"oil",ds)$(not sameas(ds,"air") AND not sameas(ds,"trn") AND not sameas(ds,"wtt") AND not sameas(ds,"trk") AND not sameas(ds,"grd") AND not sameas(ds,"pip") AND not sameas(ds,"otr")) = 
* 	id0(yr,r,"oil",ds) + (seds_oil_demand(yr,r,"oil","ldv") * (id0(yr,r,"oil",ds) / bea_dmd(yr,r,"oil")));

execute_unload '%system.fp%gdx%sep%id0test_after.gdx', id0=id0;

* cd0(yr,r,"oil") = cd0(yr,r,"oil") + seds_oil_demand(yr,r,"oil","ldv") * (cd0(yr,r,"oil") / bea_dmd(yr,r,"oil"));
* i0(yr,r,"oil") = seds_oil_demand(yr,r,"oil","ldv") * (i0(yr,r,"oil") / bea_dmd(yr,r,"oil"));
...
* shrsec(r,ioe,ds,'air')$(mapdems(ds,'air') and sum(ds.local$mapdems(ds,'air'), id0('%bnyear%',r,ioe,ds))) = id0('%bnyear%',r,ioe,ds) / sum(ds.local$mapdems(ds,'air'), id0('%bnyear%',r,ioe,ds));
* shrsec(r,ioe,ds,'train')$(mapdems(ds,'train') and sum(ds.local$mapdems(ds,'train'), id0('%bnyear%',r,ioe,ds))) = id0('%bnyear%',r,ioe,ds) / sum(ds.local$mapdems(ds,'train'), id0('%bnyear%',r,ioe,ds));
* shrsec(r,ioe,ds,'water')$(mapdems(ds,'water') and sum(ds.local$mapdems(ds,'water'), id0('%bnyear%',r,ioe,ds))) = id0('%bnyear%',r,ioe,ds) / sum(ds.local$mapdems(ds,'water'), id0('%bnyear%',r,ioe,ds));
* shrsec(r,ioe,ds,'trk')$(mapdems(ds,'trk') and sum(ds.local$mapdems(ds,'trk'), id0('%bnyear%',r,ioe,ds))) = id0('%bnyear%',r,ioe,ds) / sum(ds.local$mapdems(ds,'trk'), id0('%bnyear%',r,ioe,ds));
* shrsec(r,ioe,ds,'ldv')$(mapdems(ds,'ldv') and sum(ds.local$mapdems(ds,'ldv'), id0('%bnyear%',r,ioe,ds))) = id0('%bnyear%',r,ioe,ds) / sum(ds.local$mapdems(ds,'ldv'), id0('%bnyear%',r,ioe,ds));
...
execute_unload '%system.fp%gdx%sep%pre_balancing.gdx', id0=id0, cd0=cd0;
...
* SHANE's CHANGES BEGIN
CD.LO(r,"oil") = 0.95 * cd0loop(r,"oil");
CD.UP(r,"oil") = 1.05 * cd0loop(r,"oil");
* SHANE's CHANGES END

...
* Aggregation:
$IF NOT SET aggr $SET aggr full_disagg
...
$INCLUDE %reldir%%sep%user_defined_schemes%sep%mapseds_%aggr%.map
/;
...
* thousand kwhs for electricity. Multiplying price x quantity below
...
* using value shares for the production sector
* this method needs refining for
...
*intermediate byproducts of CNG from other sectors are split based upon the ratio of gas/cru production shares
...
$ontext
* work-in-progress:
* should we re-adjust assumptions for assume all ys0(yr,r,as,g) CNG production by-products
* would be gas-related by products?, except oil and CNG, which is split based upon shares
ys0(yr,r,'gas',g)$(NOT SAMEAS(g,'cng')) = ys0_(yr,r,'cng',g);
ys0(yr,r,'cru','oil') = shrgas(r,yr,'cru') * ys0_(yr,r,'cng','oil');
ys0(yr,r,'gas','oil') = shrgas(r,yr,'gas') * ys0_(yr,r,'cng','oil');
$offtext
...
*assume all intermediate demand values from CNG are gas, except for the crude oil going into oil refining
*these values will be re-written below with the energy demand data from SEDS.
*this process is just splitting the CNG sector, but if the gas/cru inputs are split, then that would results in smaller gas shares below when the energy data is distributed
id0(yr,r,'gas',s)$(NOT SAMEAS(s,'cng')) = id0_(yr,r,'cng',s);
id0(yr,r,'cru',s) = 0;
id0(yr,r,as,as) = sum(mapog(as,s), shrgas(r,yr,as) * id0_(yr,r,s,s));

*in SEDS, all crude only enters oil-refining sector, so taking the portion that would be assigned to own-use and adding it to refining sector supply
id0(yr,r,'cru','oil') = id0_(yr,r,'cng','oil') + id0_(yr,r,'cng','cng');
id0(yr,r,'cru','cru') = 0;
...
* Assume margins for energy is aggregated and applied uniformly to all
...
PARAMETER chkele;
chkele(r,'old') = ys0('2017',r,'ele','ele');
chkele(r,'new') = elegen(r,'total','2017') * ps0('2017','ele') / 1e3;
DISPLAY chkele;
...
* SEDS energy supply in physical units
PARAMETER   vys0, qs0, qid0, qcd0;
qs0(yr,r,'ele','ele') = elegen(r,'total',yr) ;
qs0(yr,r,'cru','cru') = sedsenergy(r,'q','cru','supply',yr) ;
qs0(yr,r,'gas','gas') = sedsenergy(r,'q','gas','supply',yr) ;
qs0(yr,r,'col','col') = sedsenergy(r,'q','col','supply',yr) ;
*There are no refined oil supply data in the SEDS file
*have to fix the following
qs0(yr,r,'oil','oil') = sedsenergy(r,'q','cru','ref',yr)/sum(r.local,sedsenergy(r,'q','cru','ref',yr)) *
    sum((demsec,r.local), sedsenergy(r,'q','cru','ref',yr));
vys0(yr,r,ioe, ioe)   =   ys0(yr,r,ioe,ioe);
qcd0(yr,r,ioe) = sum(mapioe(ioe,e), sedsenergy(r,'q',e,'res',yr));
...
PARAMETER id02017, id_ed_chk, id_ed_tab;
PARAMETER localagg;
...
localagg('2017',r,ioe,demsec)    =   sum(ds.local$mapdems(ds,demsec), id0('2017',r,ioe,ds));
...
inpchk(yr,r,ds,'bea') = sum(ioe, id0(yr,r,ioe,ds));
id02017('2017',r,ioe,ds,'bea')    =   id0('2017',r,ioe,ds);
id0(yr,r,ioe,ds) = sum(demsec, inpshrs(yr,r,ioe,demsec,ds) * sum(mapioe(ioe,e), ed0(yr,r,e,demsec)));
qid0(yr,r,ioe,ds) = sum(demsec, inpshrs(yr,r,ioe,demsec,ds) * sum(mapioe(ioe,e), eq0(yr,r,e,demsec)));

inpchk(yr,r,ds,'seds') = sum(ioe, id0(yr,r,ioe,ds));
id02017('2017',r,ioe,ds,'seds')    =   id0('2017',r,ioe,ds);


* Zero out production and allocation accounts without generation totals:
* originally just done for electricity, updated to zero out all IOE accounts without production totals
$ontext
...
ys0(yr,r,'ele',g)$(elegen(r,'total',yr) * ps0(yr,'ele') = 0) = 0;
ys0(yr,r,'cru',g)$(sedsenergy(r,'q','cru','supply',yr)*ps0(yr,'cru')=0) = 0;
ys0(yr,r,'gas',g)$(sedsenergy(r,'q','gas','supply',yr)*ps0(yr,'gas')=0) = 0;
ys0(yr,r,'col',g)$(sedsenergy(r,'q','col','supply',yr)*ps0(yr,'col')=0) = 0;
ys0(yr,r,'oil',g)$(sedsenergy(r,'q','cru','ref',yr)=0) = 0;
ys0(yr,r,'ele',as)$(elegen(r,'total',yr) * ps0(yr,'ele') = 0) = 0;
ys0(yr,r,'cru',as)$(sedsenergy(r,'q','cru','supply',yr)*ps0(yr,'cru')=0) = 0;
ys0(yr,r,'gas',as)$(sedsenergy(r,'q','gas','supply',yr)*ps0(yr,'gas')=0) = 0;
ys0(yr,r,'col',as)$(sedsenergy(r,'q','col','supply',yr)*ps0(yr,'col')=0) = 0;
ys0(yr,r,'oil',as)$(sedsenergy(r,'q','cru','ref',yr)=0) = 0;
s0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
ld0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
kd0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
ty0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
id0(yr,r,ds,ioe)$(sum(dg, ys0(yr,r,ioe,dg))=0) = 0;
id0(yr,r,as,ioe)$(sum(dg, ys0(yr,r,ioe,dg))=0) = 0;
qid0(yr,r,ds,ioe)$(sum(dg, ys0(yr,r,ioe,dg))=0) = 0;
qid0(yr,r,as,ioe)$(sum(dg, ys0(yr,r,ioe,dg))=0) = 0;
xd0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
xn0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
x0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;
rx0(yr,r,ioe)$(sum(dg, ys0(yr,r,ioe,dg)) = 0) = 0;


*   reporting parameters to check the efficacy of the zeroing out code and tabulate missing data
id02017('2017',r,ioe,ds,'seds2')    =   id0('2017',r,ioe,ds);

id_ed_chk('2017',r,ioe,demsec,'id_bea')       =   sum(ds.local$mapdems(ds,demsec), id02017('2017',r,ioe,ds,'bea'));
id_ed_chk('2017',r,ioe,demsec,'id_seds1')     =   sum(ds.local$mapdems(ds,demsec), id02017('2017',r,ioe,ds,'seds'));
id_ed_chk('2017',r,ioe,demsec,'id_seds2')     =   sum(ds.local$mapdems(ds,demsec), id02017('2017',r,ioe,ds,'seds2'));
id_ed_chk('2017',r,ioe,'res','id_bea')        =   resechk('2017',r,ioe,'old');
id_ed_chk('2017',r,ioe,demsec,'seds')         =   sum(mapioe(ioe,e),ed0('2017',r,e,demsec));
id_ed_chk('2017',r,ioe,'res','seds')          =   resechk('2017',r,ioe,'new');

id_ed_tab(r,ioe,demsec,"bea_seds")$(id_ed_chk('2017',r,ioe,demsec,'id_bea') and not id_ed_chk('2017',r,ioe,demsec,'seds'))  =   1;
id_ed_tab('sum',ioe,demsec,"bea_seds") =   sum(r, id_ed_tab(r,ioe,demsec,"bea_seds"));
id_ed_tab(r,ioe,demsec,"bea_seds2")$(id_ed_chk('2017',r,ioe,demsec,'id_bea') and not id_ed_chk('2017',r,ioe,demsec,'id_seds2'))  =   1;
id_ed_tab('sum',ioe,demsec,"bea_seds2") =   sum(r, id_ed_tab(r,ioe,demsec,"bea_seds2"));
id_ed_tab(r,ioe,demsec,"seds2_seds")$(id_ed_chk('2017',r,ioe,demsec,'id_seds2') and not id_ed_chk('2017',r,ioe,demsec,'seds'))  =   1;
id_ed_tab('sum',ioe,demsec,"seds2_seds") =   sum(r, id_ed_tab(r,ioe,demsec,"seds2_seds"));

enegdem(yr,r,ioe,'new') = sum(dg, id0(yr,r,ioe,dg)) + cd0(yr,r,ioe);
DISPLAY enegdem;

execute_unload '%reldir%%sep%temp%sep%gdx_temp%sep%kate_sedscheck.gdx' shrgas, pctgen, localagg, dataconschk, inpchk, id02017, inpshrs, ed0, id_ed_chk,id_ed_tab, qs0, qid0,qcd0;
execute_unload '%reldir%%sep%temp%sep%gdx_temp%sep%seds_supply_check.gdx' qs0, eq0, vys0;
*$exit

* calculating final energy supply and demand in physical units
...
* -------------------------------------------------------------------
EXECUTE_UNLOAD '%dsdir%%sep%WiNDC_cal_%year%_%aggr%.gdx'
